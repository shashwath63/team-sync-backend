...
var signup = (req, res) => __async(void 0, null, function* () {
  try {
    ...
    const hashedPassword = yield import_bcryptjs.default.hash(password, saltRounds);
    ...
    yield prisma2.group.create({
      data: {
        name: defaultGroupName,
        members: {
          create: { userId: user.id, role: "ADMIN" }
          // Assuming user who logged in is an admin of the group
        }
      }
    });
...
var login = (req, res) => __async(void 0, null, function* () {
  try {
    ...
    const user = yield user_default.findUnique({ where: { email } });
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }
    const passwordMatch = yield import_bcryptjs.default.compare(password, user.password);
    if (!passwordMatch) {
...
// src/routes/socket-route.ts
var import_express2 = __toESM(require("express"));

// src/controllers/socket-controller/join-group.ts
...
function joinGroup(req, res) {
  return __async(this, null, function* () {
    try {
      ...
      const decodedToken = import_jsonwebtoken2.default.verify(token, secretKey);
      const user = yield prisma3.user.findUnique({
        where: { id: decodedToken.userId },
        include: { userGroups: true }
      });
      const isMember = user == null ? void 0 : user.userGroups.some(
        (userGroup) => userGroup.groupId === groupId
      );
      if (isMember) {
        return res.status(400).json({ error: "User is already a member of the group" });
      }
      yield prisma3.userGroup.create({
        data: {
...
router2.post("/join-group", joinGroup);
var socket_route_default = router2;

// src/socket.ts
var initializeChatSocket = (io2) => {
...
app.use("/api/socket", socket_route_default);
app.get("/", (req, res) => {
